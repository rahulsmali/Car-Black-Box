...
void log_event(unsigned char event[], unsigned char speed)
{
    ...
    write_external_eeprom_string(add, log);
...
void load_password(unsigned char pwd[])
{
    ...
    for(i=0; i<4; i++)
    {
       pwd[i] = read_external_eeprom(100 + i);
    }
}
...
void save_password(unsigned char pwd[])
{
    ...
    for(i=0; i<4; i++)
    {
        ...
        write_external_eeprom(100 + i, pwd[i]);
    ...
    if(i==4)
    {
        ...
        else
        {
            ...
            if(attempt == 0)
            {
               clear_screen();
               clcd_write(0x0C, 0);
               clcd_print("Your are blocked", (0x80 + 0));
               clcd_print("wait for", (0xC0 + 0));
               secs = 60;
               while(secs != 0)
               {
                   clcd_putch(secs/10 + '0', (0xC0 + 9));
                   clcd_putch(secs%10 + '0', (0xC0 + 10));
       ...
       else if(stage == 1)
       {
           ...
           if(i==4)
           {
                ...
                else
                {
                   clear_screen();
                   clcd_write(0x0C, 0);
                   clcd_print("Pwd mismatch", (0x80 + 0));
